...
class SQLNet(nn.Module): # inheriting from parent class nn.Module
    ...
    def loss(self, score, truth_num, pred_entry, gt_where):
        ...
        if pred_agg:
            ...
            data = torch.from_numpy(np.array(agg_truth)).type(torch.LongTensor)
            if self.gpu:
        ...
        if pred_sel:
            ...
            data = torch.from_numpy(np.array(sel_truth)).type(torch.LongTensor)
            if self.gpu:
        ...
        if pred_cond:
            ...
            data = torch.from_numpy(np.array(cond_num_truth)).type(torch.LongTensor)
            if self.gpu:
            ...
            for b in range(len(truth_num)):
                if len(truth_num[b][4]) == 0:
                    continue
                data = torch.from_numpy(np.array(truth_num[b][4])).type(torch.LongTensor)
                if self.gpu:
            ...
            for b in range(len(gt_where)):
                for idx in range(len(gt_where[b])):
                    ...
                    if len(cond_str_truth) == 1:
                        continue
                    data = torch.from_numpy(np.array(cond_str_truth[1:])).type(torch.LongTensor)
                    if self.gpu:
