...
class WordEmbedding(nn.Module):
    def __init__(self, word_emb, N_word, gpu, SQL_TOK, trainable=False, word_emb_bert=None):
    ...
    def gen_x_batch(self, q, col, is_list=False, is_q=False, BERT=False):
        B = len(q)
        val_embs = []
        val_len = np.zeros(B, dtype=np.int64)
        for i, (one_q, one_col) in enumerate(zip(q, col)):
            if is_q and not BERT:
                one_qn = []
                for ql in one_q:
                    one_qnl = []
                    for q in ql:
    ...
    def str_list_to_batch(self, str_list):
        ...
        for i, one_str in enumerate(str_list):
            if self.trainable:
                # val = list(map(lambda x: self.w2i.get(x,0), one_str))
