    ...
    for i in range(st, ed):
        ...
        if db_content == 0:
            if BERT:
                ...
                q_seq_tok.append([[x] for x in sql['question_tok']])
            else:
...
def epoch_exec_acc(models, batch_size, sql_data, table_data, db_path, db_content,ensemble, BERT=False):
